...
public class Arm {
  ...
  private double TELESCPE_MINIMUM = 0; //calculate later
  private double TELESCOPE_MAXIMUM = 10; //calcate later
    ...
    public void retract(){ //arm un-telescopes
        ...
        if(telescopeMotor.getDistance() <= TELESCPE_MINIMUM){
            telescopeMotor.setPercent(0); 
        } else {
    ...
    public void telescopeToSetpoint(double meters){
        ...
        if (telescopeMotor.getDistance() >= TELESCPE_MINIMUM && telescopeMotor.getDistance() <= TELESCOPE_MAXIMUM){
            telescopeMotor.setPercent(0); 
        } else {
